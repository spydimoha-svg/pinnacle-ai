...
export type Mark = "correct" | "wrong" | "unsure";
...
function numbers(s: string): number[] {
  ...
  while ((m = re.exec(s)) !== null) {
    ...
    if (m[2]) {
      ...
      if (b !== 0) out.push(Number((a / b).toFixed(4)));
    } else {
      out.push(a);
    }
  }
...
function polarity(s: string): "yes" | "no" | null {
  ...
  if (/^\W*(?:yes|yeah|yep)\b/i.test(head)) return "yes";
  if (/^\W*(?:no|nope)\b/i.test(head)) return "no";
  if (YES.test(head) && !NO.test(head)) return "yes";
  if (NO.test(head) && !YES.test(head)) return "no";
  return null;
}
...
export function gradeAnswer(
  ...
  const cw = keyWords(correct);
  const sw = keyWords(student);
  if (!cw.size) return { mark: "unsure", why: "no stored answer to mark against" };
  let hit = 0;
  for (const w of cw) if (sw.has(w)) hit++;
  const ratio = hit / cw.size;
  if (ratio >= 0.5) return { mark: "correct", why: `matched ${Math.round(ratio * 100)}% of the answer's key terms` };
  if (ratio <= 0.15) return { mark: "wrong", why: "the answer shares almost nothing with the correct one" };
...
export function readMark(reply: string): boolean {
  const t = reply.trim().toLowerCase();
  if (/^\W*right\b/.test(t)) return true;
 ...
 */
export function stripScaffolding(text: string, opts: { keepHint?: boolean } = {}): string {

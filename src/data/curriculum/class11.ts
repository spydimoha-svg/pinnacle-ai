...
//
// weightage/boardNotes: Class XI has no external CBSE board exam, but CBSE's
// own curriculum document publishes a unit-wise Question Paper Design for the
// school-conducted annual exam (mirroring the Class XII board format), and
// schools set papers to it. Per-chapter marks below are that unit weightage
// split across the chapters within each unit; English has no official design,
// so its weightage follows the same prose > poetry convention used in the
// Class XII Flamingo chapters.

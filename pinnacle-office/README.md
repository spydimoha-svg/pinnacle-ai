...
1000 is the roster, not the concurrency. Two limits are real and neither can be
wished away:

- Every agent is a full Claude Code process. Three at once is comfortable on
  this machine, six is the ceiling.
- Only one agent may hold the codebase at a time. Two agents editing the same
  working tree makes the build gate meaningless, because neither change can be
  attributed or reverted cleanly. Advisory agents run in parallel around the
  single writer.
...
| `tell tutor rewrite the hint ladder` | give a department a job yourself, it jumps the queue |
...
1. The working tree is clean before the agent starts, so the diff is provably
   theirs. Your own uncommitted edits are parked in a commit first, never
   reverted.
...
| --- | --- | --- |
| `PINNACLE_CONCURRENCY` | 3 | agents awake at once |
